...
phoenix <- phoenix %>% mutate(grooves_pred_mad2 = purrr::map(ccdata_w_resid, .f = function(bullet){
 grooves <- get_grooves_mad(bullet, adjust = 5, multiplier = 2)$groove 
 return(grooves)
}))
...
phoenix <- phoenix %>% mutate(grooves_pred_mad4rlm = purrr::map(ccdata_w_resid, .f = function(bullet){
 grooves <- get_grooves_mad_rlm(bullet, adjust = 5, multiplier = 4)$groove 
 return(grooves)
}))
```
...
phoenix <- phoenix %>% mutate(left_groove = left_groove_kr1, right_groove = right_groove_kr1)
phoenix_eval <- prediction_scores_pho(phoenix, "mad2")
...
plot_categories(hamby44_eval, methods = c("rollapply", "mad4", "mad4rlm", "mad5"))
