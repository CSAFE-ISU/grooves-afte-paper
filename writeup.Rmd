...
nocite: | 
  @locfit, @MASS
abstract: Forensic firearms examiners analyze bullets through a process of visual feature comparison\textcolor{purple}{s} to determine whether two bullets originate from the same source \textcolor{purple}{\sout{or different sources}}. Striation marks found on land engraved areas (LEAs) provide evidence to address this same source-different source problem. Advances in technology have led to an increase in research focused on applying image-analysis algorithms to the automated, quantitative analysis of bullet evidence. One prominent example is an algorithm developed by [@Hare1] based on 3D imaging data of LEAs. This algorithm relies on removal of the overall curve of the bullet to obtain what [@Hare1] refer to as a "signature". Cross-correlation functions, consecutive matching striae, consecutive non-matching striae and other features are then calculated based on pairwise comparisons of these signatures. The currently established best practice for collection of 3D images of bullet LEAs requires capturing portions of the neighboring groove engraved areas (GEAs). \textcolor{purple}{Analyzing} LEA and GEA data separately is  \textcolor{purple}{imperative} to \textcolor{purple}{achieve} high accuracy and precision in subsequent feature calculations. However, \textcolor{purple}{existing} standard statistical modeling techniques fall short when applied to the atypical structure of 3D bullet data, often failing to adequately separate LEA and GEA data.  \textcolor{purple}{We propose a method based on robust locally weighted regregression and show that this method outperforms performs current methods at separating LEA and GEA data.} 
---
...
TODO: 
-\hh{Kiegan, could you use a color label, so I can see the changes you made in the file rather than as commit message? - I've made a kr command for you - you can pick the color :)}

- \hh{Could you remove the derivative files (tex, blg, writeup\_files, ...) from the repo? They cause a ton of merge conflicts.}

- \hh{I can't find the code for figure groove\_vs\_nogroove.png - I would like to split the figure into two subfigures (top) and (bottom) with extra lines of labelling options} 

- \hh{replace the * multiplication by cdot or times}
...
Forensic firearms examiners analyze bullets through a process of visual feature comparison\ug{s to determine whether two bullets originate from the same source}. This problem is known in forensic science as the *same source-different source problem*. In forensic firearms analysis, the same source-different source problem \ug{focuses} on \ug{establishing evidence} whether two bullets were fired through the same gun barrel.  

Striation marks, which \ug{get} engraved on the surface of a bullet through contact with the rifling of a gun barrel, provide evidence to address the same source-different source problem. Alternating sections of the bullet that make the closest contact with the barrel are called land engraved areas (LEAs). A guiding principle in forensic firearms analysis is that two bullets fired through the same barrel will bear more similar striation marks on their LEAs than two bullets fired from different barrels.   
\ug{Can you clarify or more precisely define how striation marks relate to LEAs? and then perhaps stick with one ... see my comment below.}

\ug{Striation marks are} examined using a comparison microscope: two bullets in question are placed under the microscope and firearms examiners \ug{evaluate similarities and differences between the bullets' striation marks} according to the AFTE Theory of Identification [@AFTE] \ug{guidelines resulting in a decision about whether both bullets were fired through the same gun barrel}. 

In the last \ug{two?} decades, advances in technology have led to an increase in research focused on developing image-analysis algorithms to \ug{allow for} automated, quantitative analyses of bullet evidence. The main technological \ug{advancement} that created a pathway for image-analysis techniques \ug{was?} the introduction of high resolution 3D scanning technology to the field of forensic science [e.g.\ @DeKinder1; @DeKinder2; @Bachrach1]. \ug{Add a sentence that explains what goes into the microspcope and what type of data comes out.}  The \ug{resulting}  3D data have since been used in the development of several methods of varying complexity for automated comparison of land engraved areas [e.g. @Ma1; @Chu1; @Chu2; @Hare1] \ug{Now you switched from using Striation marks as evidence to comparing land engraved areas}.  

@Hare1 proposes a \ug{bullet} matching algorithm based on 3D imaging data of LEAs. Horizontal slices of the 3D images, called profiles, provide a detailed representation of striae impressed on the surface at a horizontal cross-section of each LEA.  

Removal of the overall curve of the bullet -- the global structure captured in the 3D scanning process\ug{can we define this when we introduce the 3D data} -- transforms these profiles into to what @Hare1 refer to as *signatures*. For a land-to-land comparison \ug{comparison of two lands from two distinct 3D images?}, features such as cross-correlation function, \ug{number of?} consecutive matching striae [see @Biasotti], and consecutive non-matching striae are calculated based on the two aligned signatures. These calculated features are \ug{then} used in a random forest model to assess the \ug{degree of} similarity of the corresponding pair of signatures.   

\ug{Succesful e}xtraction of the aforementioned features depends on \ug{how well we can translate the} profile to a signature: the removal of the global structure. This step is complicated by the 3D data collection process. \ug{I would either explain why or leave the last sentence out.}  

While there is not yet an established standard, the currently \ug{accepted} best practice for collecting 3D images of bullet LEAs requires that scanning across the object must begin and end in the neighboring groove engraved areas (GEAs) as shown in \autoref{fig:LEA}. However, this practice introduces a challenging data structure. The presence of extraneous GEA data dictates the most significant step in data pre-processing: correctly identifying between data from LEAs and GEAs. 

\ug{Analyzing} these two areas separately is vital to achieve high accuracy and precision in subsequent processing steps \ug{processing what?}. Removal of data from groove engraved areas is necessary to properly calculate features used in automated comparisons. In order to distinguish between these areas, we aim to identify *shoulder locations*, the locations at which the land engraved areas end and the groove engraved areas begin.  


\section{Data Source}  


\ug{We analyze} high resolution 3D scans of bullet land engraved areas (LEAs). \ug{I would like to add an explanation of how the data exactly look like, namely in matrix format consisting of $x$, $y$ and $z$ unles you think it really is not necessary.} The scanned bullets come from Hamby Set 44 [@Hamby]. Each Hamby set consists of \ug{35 bullets fired from 10 consecutively?} rifled Ruger P85 barrels. \ug{Of the 35 bullets 20 bullets are known, two bullets for each of the ten barrels, the remaining 15 bullets in each set originated from a different source.} 


Each fired bullet in Hamby Set 44 has 6 LEAs; every LEA was scanned for each of the 35 bullets, producing data for 210 individual lands.  Two lands -- Barrel 9, Bullet 2, Land 3 and \ug{Unknowns??}, Bullet L, Land 5 -- were removed from consideration due to "tank rash". Tank rash results from a bullet striking the bottom of a water recovery tank after exiting the barrel, thereby creating marks on the land that are not due to the contact with the barrel.

The 3D scans of Hamby Set 44 were captured with a Sensofar Confocal light microscope at 20x magnification resulting in a resolution of 0.645 microns per pixel. These LEAs were scanned at Iowa State University's High Resolution Microscopy Facility, and the scans are stored in 3D format as x3p files, conforming to the ISO5436-2 standard [@ISO5436]. A visualization of the data gathered for a single LEA is seen in \autoref{fig:LEA}. Physically, each land is approximately 2 millimeters in width; as such, data structures for a single LEA can contain more than 3 million individual data points.  
...
The structure in the scans \ug{2D profiles} is dominated by the curvature of the physical object (the bullet). \ug{To assess} the  similarity of \ug{features from } two land engraved areas, this \ug{curvature} has to be removed.  

Non-parametric methods suggested in the literature, such as a LOESS fit [@Hare1] or a Gaussian filter [@Chu1] are very effective for removing the curvature, however, they are prone to \ug{so-called} boundary effects \ug{(can you give a brief definition of what the boundary effect entails in this setting?)}. In the case of bullet scans, boundary effects are exacerbated by the presence of the secondary structure: groove engraved areas at either end of the LEA (see \autoref{fig:grooveremove}).  

\hh{ describe the figure in words, in particular the boundary effect }

```{r grooveremove, echo = F, warning = F, message = F, out.width = '100%', fig.cap = "Example of non-parametric structure removal with groove engraved area data removed (top), compared to non-parametric structure removal with extraneous groove engraved area data (bottom)."}
knitr::include_graphics("images/groove_vs_nogroove.png")
```
...
Once an appropriate fit is obtained, the fitted curve is used to obtain residual values and to determine a cutoff which separates the two competing structures.  
...
The MAD is a robust metric for the spread of points, similar to the standard deviation. 
Let $m$ be the median:  
$$ MAD(\mathbf{x}) = m(|x_i- m(\mathbf{x})|) \quad \forall\ x_i \in \mathbf{x}.$$
\hh{Reduce the next paragraph to 2 sentences}
Since we are dealing with unbalanced residual values, the MAD is preferable to the standard deviation to quantify the spread. Removing unusually high residual values is effective at removing the GEA data once the global structure has been fit. Residual values that are considered unusually high, or outlying, are values that are larger than $4 \times$MAD. Any residual value larger than 4 times the median absolute deviation value is defined to be  a "large" residual.  
...
In order to assess the accuracy of the two alternative models based on a quantitative measure for their overall performance of predictions, we first manually identified "ground truth" shoulder locations. \hh{on which data? tie it back to the data section}  
...
The raw distributions can be difficult to visually compare, so another way to inspect the results is to place inaccuracy scores into categories: satisfactory, borderline, unsatisfactory. Scores under 100 are satisfactory, scores between 100 and 1000 are borderline, and scores above 1000 are unsatisfactory (see \autoref{results2}). Unsatisfactory cases are the most likely to cause mistakes in subsequent analyses. 
