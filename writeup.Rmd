...
Removal of the overall curve of the bullet -- the global structure captured in the 3D scanning process\ug{can we define this when we introduce the 3D data} -- transforms these profiles into to what @Hare1 refer to as *signatures*. For a land-to-land comparison \ug{comparison of two lands from two distinct 3D images?}, features such as cross-correlation function, number of consecutively matching striae [see @Biasotti], and maximum number of consecutively non-matching striae are calculated based on the two aligned signatures. These calculated features are then used in a random forest model to assess the degree of similarity of the corresponding pair of signatures.   

Succesful extraction of the aforementioned features depends on how well we can translate the profile to a signature: the removal of the global structure. This step is complicated by the 3D data collection process. \ug{I would either explain why or leave the last sentence out.}  
