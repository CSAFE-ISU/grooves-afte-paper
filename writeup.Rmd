...
\ug{Can you clarify or more precisely define how striation marks relate to LEAs? and then perhaps stick with one ... see my comment below.} \kr{I think it's in there above.}  

Land engraved areas are examined using a comparison microscope: two bullets in question are placed under the microscope and firearms examiners \ug{evaluate similarities and differences between the bullets' striation marks} according to the AFTE Theory of Identification [@AFTE] \ug{guidelines resulting in a decision about whether both bullets were fired through the same gun barrel}. 

Recent advances in technology, particularly wider access to high resolution 3D microscopy tools, have led to an increase in research focused on image-analysis algorithms for automated, quantitative analyses of bullet evidence. The introduction of this scanning technology to the field of forensic science allows for capture of high resolution 3D images of bullet LEAs [see\ @DeKinder1; @DeKinder2; @Bachrach1]. \ug{Add a sentence that explains what goes into the microspcope and what type of data comes out.}  The \ug{resulting} 3D data have since been used in the development of several methods for automated comparison of land engraved areas [e.g. @Ma1; @Chu1; @Chu2; @Hare1] \ug{Now you switched from using Striation marks as evidence to comparing land engraved areas}. \kr{I switched most things to LEA now instead.}  
...
Successful extraction of the aforementioned features depends on how well we can translate the profile to a signature: the removal of the global structure.}  
...
The 3D scans of Hamby Set 44 were captured with a Sensofar Confocal light microscope at 20x magnification resulting in a resolution of 0.645 microns per pixel. These LEAs were scanned at Iowa State University's High Resolution Microscopy Facility, and the scans are stored in 3D format as x3p files, conforming to the ISO5436-2 standard [@ISO5436]. \kr{Scan data for each LEA are captured in a matrix of $(x,y)$ locations, with a measured relative height value $z$ recorded for each $(x,y)$ location on the physical object.} A visualization of the data gathered for a single LEA is seen in \autoref{fig:LEA}. Physically, each land is approximately 2 millimeters in width; as such, data structures for a single LEA can contain more than 3 million individual data points.  
