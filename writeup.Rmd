...
All currently published automated methods rely on high resolution 3D scans of bullet land engraved areas. Currently accepted best practice for collecting 3D images of bullet LEAs requires that bullets are staged such that striae appear vertically in the scan. Scanning across the LEA must begin and end in the neighboring groove engraved areas as shown in \autoref{fig:LEA}. Parts of the breakoff \ug{(define?)} are captured as a visual reference for orientation.  
...
Scans are exported from the microscope as x3p files, conforming to the ISO5436-2 standard [@ISO5436]. Each scan is recorded as a \ug{3D?} matrix of \ug{prespecified?} $(x,y)$ locations with a measured relative height value $z$ for each $(x,y)$ location on the LEA. 
...
\end{minipage}
\caption{The process of extracting a 2D signature from a 3D LEA scan described by \cite{Hare1}. GEA removal between Step\ug{s} 2 and 3 is critical to ensure precise signature extraction.}  
\label{fig:process}
\end{figure}


In th\ug{e following}, we are introducing and comparing two methods for identifying shoulder locations. In order to assess the performance of these methods, we are applying \ug{both} methods on 3D scans of LEAs from Hamby set 44 [@Hamby]. Each Hamby set consists of 35 bullets fired from 10 consecutively rifled Ruger P85 barrels. 
...
\ug{All} 3D scans of Hamby Set 44 were captured at Iowa State University's Roy J.\ Carver High Resolution Microscopy Facility with a Sensofar Confocal light microscope at 20 \ug{times} magnification resulting in a resolution of 0.645 microns per pixel. Physically, each land is approximately 2 millimeters in width; as such, data structures for a single LEA can contain more than 3 million individual data points. 



```{r prof, echo = F, warning = F, message = F, fig.height = 3.3, fig.cap = "\\label{prof}The black points show measured heights for a single crosscut of a 3D LEA scan. The main data structure, located in the center, is comprised of the land engraved area. The groove engraved areas are found on the left and right sides of the crosscut. The lines show fits of two non-parametric LOESS smooths, with and without GEA data. When GEA data is included, the smooth fails to estimate the main LEA structure near the boundaries."}
...
\ug{Data} used to assess performance of the two methods consist of 2D crosscuts gathered from the 3D scans, as shown in \autoref{prof}. 


\section{Methodology}  


The structure in the 2D crosscuts is dominated by the curvature of the physical object (the bullet). To assess the similarity of features from two land engraved areas, this curvature has to be removed.  

Non-parametric methods suggested in the literature, such as a LOESS fit [@Hare1] or a Gaussian filter [@Chu1] are effective for removing the curvature to extract a signature. However, they are prone to boundary effects, which cause mischaracterizations of data patterns near the boundaries of the data domain. In the case of crosscuts, the boundaries are often dominated by values originating from the GEA structure. This structure exaggerates existing boundary effects because GEAs introduce a secondary structure different from the main curvature of the LEA, as shown in \autoref{prof} and \autoref{groove-no-groove}. \autoref{prof} shows how much a non-parametric LOESS fit is affected by including GEA data. \autoref{groove-no-groove} illustrates the effect th\ug{e inclusion of the same} has on extracted signatures. If included, GEA data result in strong boundary effects in the signatures. Statistically, GEA data introduce outliers into the LEA data. In the next sections, we introduce two methods for fitting the LEA structure. Both methods aim to describe the relationship between horizontal position and relative height on a crosscut. While the two approaches differ in methodology, they are both rooted in the ability to mitigate \ug{potential} influence caused by outlying data.  


```{r groove-no-groove, echo = F, warning = F, message = F, fig.cap = "\\label{groove-no-groove}An example of the impact failure to remove GEA data can have on an extracted 2D signature. Even though there are only very few points in the GEA structure, the extracted signatures are dominated by boundary effects.", fig.height=4, fig.width = 6}
library(latex2exp)
...
prof_1 <- bullet_sig %>% 
  ...
  theme_bw() + 
  labs(x = TeX("Relative Location $x_i$"), y = TeX("Relative Height $z_i$"), 
...
sig_1 <- bullet_sig %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = TeX("Relative Location $x_i$"), y = "Signature Height",
...
$$
where all error terms $\epsilon_i$ are considered independent and normally distributed with mean $0$ and variance $\sigma^2$. \ug{Parameter values of} $\beta_0, \beta_1, \beta_2$ are estimated by finding the values which minimize: 
...
p1_rlm <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Relative Height (", mu, "m)")), title = "(a) Linear Model Fit") + 
...
p2_rlm <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Residual Height (", mu, "m)")), title = "(b) Linear Model Residuals") + 
...
p3_rlm <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Relative Height (", mu, "m)")), title = "(c) Robust Linear Model Fit") + 
...
p4_rlm <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Residual Height (", mu, "m)")), title = "(d) Robust Linear Model Residuals") + 
  geom_hline(yintercept = 0, color = "red") +
  theme(axis.title.x = element_text(size = rel(1)), 
        axis.title.y = element_text(size = rel(1)), 
        axis.text.x = element_text(size = rel(1)),
...
The MAD is a robust metric for the spread of \ug{data}, similar to the standard deviation. It is preferable to the standard deviation to quantify the spread in situations with large outlying \ug{observations such as the residuals in the groove engraved area in our case}.  

Let $m$ denote the median function\ug{. Then the MAD is defined as }  
...
\item Calculate the \ug{residual} median absolute deviation (MAD) for the crosscut.  
...
where $w_k(x_i)$ is a weight assigned to each data point $x_k$ based on \ug{its} proximity to $x_i$. Weights $w_k$ decrease as the distance to $x_i$ increases, so that data points closest to $x_i$ influence the prediction $\widehat{z}_i$ most. This \ug{? more clear if specified} can also be described as a non-parametric weighted average of many parametric models fit to subsets of the data.  

\ug{Although t}his approach allows for greater flexibility\ug{,}} LOESS models are affected \ug{more}  by GEA structures \ug{than what? (robust) linear models?}. Data points near and in the GEA structure are most influenced by other GEA data rather than the overall global structure. This results in a set of predictions which misrepresents much of the data near the boundaries (see \autoref{loess}).

Similar to the linear approach, there \ug{exists} a robust approach to LOESS to \ug{adjust for} these boundary effects. 

The robust approach to LOESS uses an iterative re-weighting process to reduce the influence of outlying data points [see @Cleveland1]. First, an initial LOESS is fit. This \ug{step} is followed by a redistribution of the weights $w_k(x_i)$ based on residual values, $e_i = (z_i - \widehat{z}_i)$. New weights are calculated as 
$$\left(1-\left(\frac{e_k}{6\times MAD}\right)^2\right)^2 \times w_k(x_i) \quad \quad \mbox{if}\quad \ \left|\frac{e_k}{6\times MAD}\right| < 1,$$
\ug{else}, weights are set to 0. These new weights are \ug{re-applied in (\ref{eq:RobLOESS})} and updated predictions are \ug{obtained}. This reduces the influence of data points with large residual values $e_k$ in subsequent iterations. In the context of LEA crosscuts, this re-weighting reduces the influence of GEA data.


\ug{Not sure the following sentence/paragraph expresses clearly what I think you want to say.} While robust LOESS methods are more flexible than robust linear models, a model that is accurately fit to the LEA structure \ug{still} results in the same expected residual structure as with robust linear models: positive and negative residuals scattered around zero in the land engraved areas, and positive, possibly large residuals in the groove engraved areas. We are using a similar approach as with the linear shoulder location predictions, i.e., we use a cutoff value to identify large magnitude residual values. The LOESS approach fits more closely to the crosscut and allows us to choose a lower cutoff for separation. A cutoff that performs well on the Hamby set 44 is twice the median absolute deviation ($2\times MAD$). 
...
p1_rlo <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Relative Height (", mu, "m)")), title = "(a) LOESS Fit") + 
...
p2_rlo <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Residual Height (", mu, "m)")), title = "(b) LOESS Residuals") + 
  geom_hline(yintercept = 0, color = "red") +
  theme(axis.title.x = element_text(size = rel(1)), 
        axis.title.y = element_text(size = rel(1)), 
        axis.text.x = element_text(size = rel(1)),
...
p3_rlo <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Relative Height (", mu, "m)")), title = "(c) Robust LOESS Fit") + 
...
p4_rlo <- bullet %>% ggplot() + 
  ...
  theme_bw() + 
  labs(x = expression(paste("Relative Location (", mu, "m)")), 
       y = expression(paste("Residual Height (", mu, "m)")), title = "(d) Robust LOESS Residuals") + 
  geom_hline(yintercept = 0, color = "red") +
  theme(axis.title.x = element_text(size = rel(1)), 
        axis.title.y = element_text(size = rel(1)), 
        axis.text.x = element_text(size = rel(1)),
...
\begin{enumerate}
\item Fit a robust LOESS model with \ug{a span of 1?, not mentioned before and possibly uninformative to some readers} to the crosscut. This can be fit using the `locfit.robust` function in the `locfit` package in R. \ug{(Is span=1 by any chance the default setting?)}
\item Calculate a residual value $e_i$ for each data point on the crosscut.  
\item Calculate the \ug{residual} median absolute deviation (MAD) for the crosscut.  
...
\ug{T}o \ug{quantitatively} assess the \ug{predictive ability} of the two alternative models, we first  identified "ground truth" of shoulder locations by visual inspection for each of the 208 crosscuts in Hamby set 44.


\ug{As a} performance \ug{measure}, we define the error as the "area of misidentification", i.e., the area of each crosscut which is identified incorrectly by the method. This metric is calculated for the left shoulder location as:  

$$ \widehat{A}_{jL} = \sum_{e_{ij} \in \widetilde{X}_{jL}} \left|e_{ij} \right| \times \left(x_{i+1} - x_i \right),$$
where $\widetilde{X}_{jL}$ is the set of points in crosscut $j$ that fall between the predicted and actual left shoulder location, $e_{ij}$ \ug{corresponds to} the residual value at location $x_i$ from the robust LOESS fit to crosscut $j$, and $(x_{i+1} - x_i)$ is the distance between two subsequent locations in the crosscut. For the scans of Hamby set 44, this distance is equal to $0.645 \mu m$ for all locations.  

\ug{Similarly, the} area is calculated for the right shoulder location as: 
...
\ug{Quantifying the}  results as an area is preferable to a distance metric as it captures not only the width of \ug{the} profile area that is misidentified, but also the relative heights of the data. Larger areas of misidentification \ug{(Larger misidentified areas) indicate larger portions of the GEA remain included in a profile, and thus signal an area which is more likely to have influence on an extracted signature. Smaller areas of misidentification \ug{suggest} minimal loss is incurred, and these areas will have minimal effect on an extracted signature.  

An area of misidentification was calculated separately for the left hand side and right hand side predictions for each \ug{of the 208} profile\ug{s} in the data set\ug{, where predictions were based on} the Robust Linear Model and Robust LOESS methods, as well as the Rollapply method suggested in @Hare1.  
...
To assess the performance of \ug{all three} methods, we \ug{consider} the distribution of the \kr{areas of misidentification} across all 208 lands of Hamby set 44 (see \autoref{results1}). A distribution \ug{starting at or close to zero with minimal} spread is ideal \ug{as} this suggests many of the predicted shoulder locations are very close to the manually identified locations, and predictions are removing many of the outlying GEA points. A distribution with a wider spread or many high, outlying areas of misidentification suggests a greater degree of uncertainty and inaccuracy for a particular method.  
...
\ug{Because the} raw distributions \ug{are} difficult to visually compare, \ug{we categorized} areas of misidentification \ug{as} satisfactory, borderline, \ug{and} unsatisfactory. Scores under 100 are \ug{considered} satisfactory, scores between 100 and 1000 are borderline, and scores above 1000 are \ug{considered} unsatisfactory (see \autoref{results2}). Unsatisfactory cases are the most likely to cause \ug{poor or flawed resutls} in subsequent analyses. 
