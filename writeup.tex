...
striation marks on land engraved areas of two bullets when investigating
whether both bullets were propelled through the same gun barrel.
...
scanning technology to the field of forensic science
...
The nature of the 3D scanning process for land engraved areas (LEAs)
introduces a challenging data pre-processing problem. \emph{HH: what you
describe is part of the currently established best practice (there's no
standard operating procedure yet, but NIST is working on one)} To
guarantee capture of an entire land engraved area, scanning across the
object must begin and end in the neighboring groove engraved areas
(GEAs). \emph{HH: you could refer to figure 1 here} \emph{HH: what does
the `this' refer to?}This ensures that the maximal amount of land
surface area can be utilized in image-analysis methods, providing the
most reliable feature generation and more robust results. \emph{HH:
`most reliable' - we don't know that, how about we `aim for reliability
and robustness in the subsequent feature generation'} This extraneous
data collection dictates the most significant step in data
