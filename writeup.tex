...
\emph{HH: the paper needs a bit more intro. Start with the more general
problem of comparisons to determine same source/different source. Define
land engraved area and describe how we get to striae.}
...
\emph{HH: the comparison is not necessarily between a known and an
unknown bullet, it could also be between two unknown bullets }
...
Following the 2009 report, researchers \emph{HH `researchers' is too
vague, add citations; it also implies that research has only started in
2009. that's not true.} began more intensely studying the validity of
feature comparison methods as well as investigating the feasibility of
developing image-analysis algorithms to complete automated, quantitative
analyses. The main technological development that has created a pathway
for image-analysis techniques is the introduction of high resolution 3D
scanning technology to the field of forensic science. \emph{HH: add
citations}
...
\emph{HH: the following paragraph breaks the flow. move up.} Criticisms
of firearms examination in recent years have focused on foundational
validity and reliability \citep[e.g.][]{PCAST2016}. These criticisms
enforce the need for automated algorithms to undergo careful study and
validation before they can be reasonably implemented to assist forensic
firearms examiners. This process of algorithm development includes data
pre-processing methods \textbf{that ensure the correct data are being
used} \emph{HH: that's not what pre-processing does} in automated
methods.

\emph{HH: All of the discussion of the next two paragraphs depends on
the matching algorithm in \citet{Hare1}. You should go into more detail
on the algorithm first. }

\sout{Data pre-processing is not usually considered a significant
barrier for most research endeavors. However,} the nature of the 3D
scanning process for land engraved areas (LEAs) introduces a challenging
data pre-processing problem. \emph{HH: what you describe is part of the
currently established best practice (there's no standard operating
procedure yet, but NIST is working on one)} To guarantee capture of an
entire land engraved area, scanning across the object must begin and end
in the neighboring groove engraved areas (GEAs). \emph{HH: you could
refer to figure 1 here} \emph{HH: what does the `this' refer to?}This
ensures that the maximal amount of land surface area can be utilized in
image-analysis methods, providing the most reliable feature generation
and more robust results. \emph{HH: `most reliable' - we don't know that,
how about we `aim for reliability and robustness in the subsequent
feature generation'} This extraneous data collection, \sout{while
necessary}, dictates the most significant step in data pre-processing:
correctly identifying between data from LEAs and GEAs.

Dealing with these two areas separately is crucial to ensure accuracy
and precision in subsequent processing steps. \emph{HH: `ensure' is too
much - we don't know} Removal of data from groove engraved areas
significantly reduces the possibility of misidentification of the
characteristics used in automated comparisons. \emph{HH: `significantly
reduces' - if you state that we need to show it. I am not sure what you
mean by `possibility of misidentification'} In order to distinguish
between these areas, we aim to identify ``shoulder locations'', the
locations at which the LEA ends and the GEAs begin.
...
\begin{figure}
\centering
\includegraphics[width=6.25in,height=\textheight]{./images/3d_plot_top.png}
...
here. \emph{HH: include a sketch of a bullet with a marker of which area
is included. Why did you decide on going to a grey scale for the
figure?}}
\end{figure}
...
when a data-based Consecutively Matching Striae (CMS) \emph{HH: cite
biasotti} measure is calculated as part of the algorithm. This
striation-focused approach suggests mainly utilizing horizontal slices
of the 3D scan, called crosscuts, that capture the striation pattern
